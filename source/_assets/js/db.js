...
async function initDB(bool = false) {
  ...
  request.onsuccess = async function (event) {
    ...
    if (
      ...
      !db.objectStoreNames.contains("supportedMods")
    ) {
      db.close();
      deleteIndexedDB();
      createDBs(db);
...
function createDBs(db) {
  if (!db.objectStoreNames.contains("assetData")) {
    let assetData = db.createObjectStore("assetData", {
      keyPath: "PrefabID",
    });
    assetData.createIndex("PrefabID", "PrefabID", { unique: true });
    assetData.createIndex("GUID", "GUID", { unique: true });
  }

  if (!db.objectStoreNames.contains("langData")) {
    let langData = db.createObjectStore("langData", {
      keyPath: "key",
    });
    langData.createIndex("key", "key", { unique: true });
  }

  if (!db.objectStoreNames.contains("supportedMods")) {
    let modData = db.createObjectStore("supportedMods", {
      keyPath: "mod",
    });
    modData.createIndex("mod", "mod", { unique: true });
  }
  localStorage.removeItem("trackerUpdatedAt");
}
...
async function addAssetDataAll(data) {
  if (!Array.isArray(data)) {
    console.error("Expected array for asset data:", data);
    return;
  }
  let transaction = db.transaction(["assetData"], "readwrite");
  // let transaction_t = db.transaction(["timeSince"], "readwrite");
...
async function addLangDataAll(data) {
  if (typeof data !== "object" || Array.isArray(data)) {
    console.error("Expected array for lang data:", data);
    return;
  }
...
async function addSupportedModsAll(data) {
  if (!Array.isArray(data)) {
    console.error("Expected array for supported mods data:", data);
    return;
  }
  let transaction = db.transaction(["supportedMods"], "readwrite");
  // let transaction_t = db.transaction(["timeSince"], "readwrite");
  ...
  let objectStore = transaction.objectStore("supportedMods");

  // console.log(data);
  // let objectStore_t = transaction_t.objectStore("timeSince");
  data.forEach((mod) => {
    objectStore.put({ mod: mod });
  });
  // objectStore_t.put({
  //   name: `langData`,
  //   time: new Date().getTime(),
...
window.addSupportedModsAll = addSupportedModsAll;
