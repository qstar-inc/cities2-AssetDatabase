...
async function startInterface() {
  ...
  document.querySelector(".options-dropdown-trigger-text").innerText =
    await getLangData("lang_title");
}

const optionInterfaceLanguage = document.getElementById(
...
if (optionInterfaceLanguage && optionRightSubTitle && optionRightSubDesc) {
  optionInterfaceLanguage.addEventListener("mouseover", async () => {
    optionRightSubTitle.innerText = await getLangData(
      "Options.OPTION[InterfaceSettings.currentLocale]"
    );
    optionRightSubDesc.innerText = await getLangData(
      "Options.OPTION_DESCRIPTION[InterfaceSettings.currentLocale]"
    );
  });
...
const optionDropdownTrigger = document.querySelector(
  ".options-dropdown-trigger"
);
if (optionDropdownTrigger) {
  const blockEverything = document.getElementById(
    "option-dropdown-select-lang-block-everything"
  );
...
if (optionHofEnable && optionRightSubTitle && optionRightSubDesc) {
  ...
  const optionHofEnableCheckbox = document.getElementById(
    "option-hof-enable-checkbox"
  );
