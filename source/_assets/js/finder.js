...
const formHandler = async (e) => {
    ...
    if (userInput.length === 0) {
        ...
        searchResults.innerHTML = `${repeat("&nbsp;", 5)}${await getTranslation('input-3-characters')}`;
        return;
    }
    debounceTimeout = setTimeout(async () => {
        ...
        let filtered;
        if (searchType == "name") {
            filtered = searched.filter(prefab => prefab.PrefabID.toLowerCase().includes(userInput)).sort();
        } else {
            filtered = searched.filter(prefab => prefab.GUID.toLowerCase().includes(userInput)).sort();
        }
